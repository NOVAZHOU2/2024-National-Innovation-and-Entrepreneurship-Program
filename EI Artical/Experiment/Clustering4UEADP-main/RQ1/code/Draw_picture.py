...
class BoxPlot:
    def __init__(self, phome):
        self.color_dict = {1: 'red', 2: "green", 3: 'blue', 4: 'orange', 5: 'purple', 6: 'yellow', 7: 'pink', 8: 'gray',
                           ...
                           16: 'gray', 17: 'gray', 18: 'gray', 19: 'gray', 20: 'gray', 21: 'gray', 22: 'gray', 23: 'gray', 24: 'gray', 25: 'gray'
                           }
    ...
    def parseTxtToCSV(self, header, txt_path):
        ...
        for head in header:
            # 获取 header 中每个方法名称的前缀（即 '.' 前面的部分）
            head_prefix = head.split('.')[0]
            for i, method_name in enumerate(method):
                # 获取 method 中方法名称的前缀
                method_prefix = method_name.split('.')[0]
                if head_prefix == method_prefix:
                    #print(head)  # 打印匹配的 header 项
                    header_rank.append(rank[i])
                    #print(rank[i],end=' ')
                    break  # 找到匹配项后跳出内层循环
        data = []
        data.append(header)
    ...
    def parse(self):
        ...
        for file in os.listdir(results_path):
            ...
            # GBC: bang
            header = [
                      'Kmeans.LOC', 'Xmeans.NPM', 'Fcm.Ce', 'Gmeans.LCOM3', 'MiniBatchKmeans.LOC',
                      'Birch.Ce',  'Rock.AMC', "Agglomerative.LOC",
                      "Dbscan.LOC", 'Hdbscan.CBO', 'Optics.LOC', 'MeanShift.LCOM',
            ...
            new_header = [
                'K-means.LOC', 'X-means.NPM', 'FCM.Ce', 'G-means.LCOM3', 'MiniBatchKmeans.LOC',

                'BIRCH.Ce',  'ROCK.AMC', "AHC.LOC",
                "DBSCAN.LOC", 'HDBSCAN.CBO', 'OPTICS.LOC', 'MeanShift.LCOM',
            ...
            plt.axvline(15.5, color='black', linestyle=':')
            plt.axvline(16.5, color='black', linestyle=':')
            plt.axvline(19.5, color='black', linestyle=':')
