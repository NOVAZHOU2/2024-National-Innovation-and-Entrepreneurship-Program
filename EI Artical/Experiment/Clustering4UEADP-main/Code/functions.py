...
from sklearn.cluster import KMeans, AgglomerativeClustering, Birch, MiniBatchKMeans, MeanShift, AffinityPropagation, SpectralClustering
...
# 就是直接利用了pyclustering和sklearn里面的聚类，直接去返回预测值
# 因为这些都是无监督的聚类方法，所以都是直接用的测试集数据
...
def KmeansCluster(n, X):
    ...
    # KMeans(n)：n代表数据集将被分为n个簇
    # KMeans的fit_predict(X)：计算簇的中心并且预测每个样本对应的簇的类别
    # 会返回labels标签（0， 1， 2……），也就是每个点的归属簇
    return pred, fname
...
def BirchCluster(n, X):
    fname = 'Birch'
    pred = Birch(n_clusters=n).fit_predict(X)
    return pred, fname
...
def HdbScan(n, X):
    fname = 'HDBSCAN'
    pred = hdbscan.HDBSCAN(min_cluster_size=n).fit_predict(X)
    return pred, fname


def Spectral(n, X):
    fname = 'Spectral_Clustering'
    pred = SpectralClustering(n_clusters=n).fit_predict(X)
    return pred, fname

def gaussianMixture(n, X):
    fname = 'GaussianMixture'
    pred = GaussianMixture(n_components=n).fit_predict(X)
    return pred, fname

def isolationForest(n, X):
    fname = 'isolationForest'
    pred = IsolationForest().fit_predict(X)
    return pred, fname