...
class BoxPlot:
    ...
    def parseTxtToCSV(self, header, txt_path):
        ...
        if not os.path.exists(txt_path):
            print("txt文件：{0}不存在".format(txt_path))
            exit()
        method = []
        rank = []
    ...
    def parse(self):
        ...
        for file in os.listdir(results_path):
            ...
            header = ['Kmeans', 'Kmedoids', 'Xmeans', 'FCM', 'Gmeans', 'MBKmeans', 'Kmeans..',
                      'BIRCH', 'CURE', 'ROCK', 'AHC',
            ...
            csv_name = file[:-4]
            csv_path = '../Result_/{0}'.format(csv_name)
            ...
            new_header = ['Kmeans', 'Kmedoids', 'Xmeans', 'FCM', 'Gmeans', 'MBKmeans', 'Kmeans + +',
                      'BIRCH', 'CURE', 'ROCK', 'AHC',
                      'DBSCAN', 'OPTICS', 'MeanShift',
                      'SOMAC', 'SYNCSOM', 'EMA',
                      'AP',
            ...
            output_file_path = '../pictures/DIS_{0}.png'.format(csv_name[:-5])
            foo_fig = plt.gcf()
